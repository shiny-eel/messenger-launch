...
let _myPeople: Person[] = []; // Let my people go!
...
function goToMessengerTab(text: string, callback: (tab: any) => any) {
    ...
    chrome.tabs.query({
        ...
        if (tabs.length === 1) { // Already have 1 tab
            // var tabID = tabs[0].id;
            let newId = tabs[0].id
            favouriteTabId = (newId !== undefined) ? newId : favouriteTabId;
...
function switchToPerson(tabId: number): void {
    console.log("Calling editMessenger.js script to navigate to correct chat.");
...
function saveNewPerson(tab: Tab) {
    ...
    if (username && !hasPersonAlready(username)) {
        console.log("Updating the parasite to save another person.");
...
function hasPersonAlready(username: string) {
    if (!_myPeople)
        return false;
    for (let i = 0; i < _myPeople.length; i++) {
        if (_myPeople[i].username == username) {
...
function listenToMessengerPage() {
    ...
    console.log("BACKGROUND: Starting the tab updated listener.");
    chrome.tabs.onUpdated.addListener(function (_tabId, info, tab) {
        // console.log("Messenger tab changed/opened.", info);
        if (tab.url && !tab.url.match(fbMessengerURL)) return;

        if (info.status === "complete") saveNewPerson(tab);
